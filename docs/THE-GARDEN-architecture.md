...
Each pipeline module has a defined input/output contract. If a module cannot be called with these inputs and return these outputs, the pipeline is broken.
...
Known gap: W2 — temp_f/wind_mph/description null, afd_summary null, only 7 forecast periods
...
Known gap: A1 — prompt uses coarse season bucket not sekki; term_cue = first sentence only
